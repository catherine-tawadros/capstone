...
class Regularizer:
    ...
    def l1_grad(self, weights):
        ...
        """
        regularize = weights[1:,:]
        return np.concatenate([np.zeros((1,weights.shape[1])), np.where(regularize != 0, regularize/np.abs(regularize), 0)]) * self.alpha
    ...
    def l2_grad(self, weights):
        ...
        """
        regularize = weights[1:,:]
        return np.concatenate([np.zeros((1,weights.shape[1])), regularize]) * 2 * self.alpha
